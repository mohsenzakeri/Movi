...
MoveRow::MoveRow(uint64_t p_, uint16_t n_, uint64_t pp_, uint16_t offset_, uint64_t id_) {
    this->init(p_, n_, pp_, offset_, id_);
}

void MoveRow::init(uint64_t p_, uint16_t n_, uint64_t pp_, uint16_t offset_, uint64_t id_) {
...
void MoveRow::init(uint64_t p_, uint16_t n_, uint64_t pp_, uint16_t offset_, uint64_t id_, char c_) {
    overflow_bits = std::numeric_limits<uint32_t>::max();
    this->set_p(p_);
    this->set_n(n_);
    this->set_pp(pp_);
