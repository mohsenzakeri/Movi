...
#if TALLY_MODES
MoveRowColored::MoveRowColored(uint16_t n_, uint16_t offset_) {
    this->init(n_, offset_);
}
#else
MoveRowColored::MoveRowColored(uint16_t n_, uint16_t offset_, uint64_t id_) {
    this->init(n_, offset_, id_);
}
#endif

#if TALLY_MODES
void MoveRowColored::init(uint16_t n_, uint16_t offset_) {
#else
void MoveRowColored::init(uint16_t n_, uint16_t offset_, uint64_t id_) {
    ...
    this->set_id(id_);
    if (id_ != this->get_id()) {
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - init] The id setter or getter is not working properly.\n" +
    ...
    this->set_n(n_);
    if (n_ != this->get_n()) {
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - init] The length setter or getter is not working properly.\n" +
                                           "n_: " + std::to_string(n_) +
                                           " this->get_n(): " + std::to_string(this->get_n())));
    }
    this->set_offset(offset_);
    if (offset_ != this->get_offset()) {
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - init] The offset setter or getter is not working properly.\n" +
                                           "offset_: " + std::to_string(offset_) +
                                           " this->get_offset(): " + std::to_string(this->get_offset())));
    }
}

std::ostream& operator<<(std::ostream& os, const MoveRowColored& mr)
{
#if TALLY_MODES
...
#if SPLIT_THRESHOLDS_FALSE
void MoveRowColored::set_threshold_status(uint16_t i, uint8_t status) {
...
void MoveRowColored::set_overflow_thresholds() {
    overflow_bits = overflow_bits & mask_overflow_thresholds;
    overflow_bits = overflow_bits | (1U >> 6);
}

void MoveRowColored::set_n(uint16_t n_) {
    n = n_;
}

void MoveRowColored::set_overflow_n() {
    overflow_bits = overflow_bits & mask_overflow_n;
    overflow_bits = overflow_bits | (1U >> 4);
}

void MoveRowColored::set_offset(uint16_t offset_) {
    offset = offset_;
}

void MoveRowColored::set_overflow_offset() {
    overflow_bits = overflow_bits & mask_overflow_offset;
    overflow_bits = overflow_bits | (1U >> 5);
}


void MoveRowColored::set_id(uint64_t id_) {
...
void MoveRowColored::set_c(char c_, std::vector<uint64_t>& alphamap) {
...
void MoveRowColored::set_n(uint16_t n_) {
    ...
    if (n_ < (1U << LENGTH_BITS)) {
        ...
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - set_n] The length is greater than " + std::to_string(MAX_RUN_LENGTH) + ": " + std::to_string(n_)));
    }
}
...
void MoveRowColored::set_offset(uint16_t offset_) {
    ...
    if (offset_ < (1U << LENGTH_BITS)) {
        ...
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - set_offset] The offset is greater than " + std::to_string(MAX_RUN_LENGTH) + ": " + std::to_string(offset_)));
    }
}
...
#if USE_THRESHOLDS
void MoveRowColored::set_threshold(uint16_t i, uint16_t value) {
    if (value > 1) {
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - set_threshold] The theshold may be either 0 or 1: " + std::to_string(i)));
    }
    switch (i) {
        case 0:
        ...
        case 2:
            ...
            break;
        default:
            throw std::runtime_error(ERROR_MSG("[MoveRowColored - set_threshold] Only three thresholds may be stored: " + std::to_string(i)));
    }
}
#endif
#endif
...
void MoveRowColored::set_n(uint16_t n_) {
    if (n_ <= MAX_RUN_LENGTH) {
        ...
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - set_n] The length is greater than " +
...
void MoveRowColored::set_offset(uint16_t offset_) {
    if (offset_ <= MAX_RUN_LENGTH) {
        ...
        throw std::runtime_error(ERROR_MSG("[MoveRowColored - set_offset] The offset is greater than " +
